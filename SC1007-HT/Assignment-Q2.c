...
// DOUBLY LINKED LIST FOR CLOSE ADDRESSING HASHTABLE
...
int HashInsert(HashTable *Q1Ptr, int key)
{
    ...
    ListNode *newNode;
    // duplicate
    if (HashSearch(*Q1Ptr, key) != NULL || Q1Ptr->hSize == 0)
        return 0;
    // get hashslot
    if (Q1Ptr->hSize != 0)
    ...
    // insert new node to front of hash slot LL
...
int HashDelete(HashTable *Q1Ptr, int key)
{
    ...
    // if head is the key to be deleted -> need to change head
    if (temp->key == key)
    {
